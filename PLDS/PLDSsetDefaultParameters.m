function params = PLDSsetDefaultParameters(params,xDim,yDim,nACC)
%
...
if(xDim==2)
    params.model.notes.useCMask=true;
    if(params.opts.doubleRegion==1)
        params.model = touchField(params.model,'CMask',[ones(nACC,1) zeros(nACC,1);zeros(yDim-nACC,1) ones(yDim-nACC,1)]);
    else
        params.model = touchField(params.model,'CMask',[ones(yDim,1) zeros(yDim,1)]);
    end
end
