...
for tr = 1:Trials
    ...
    S11                = S11                + sum(Vsm(:,:,2:T),3)  + MUsm1*MUsm1';  % Wk+1=pow(sigma,2)+pow(xk,2)
    S01(1:xDim,:)      = S01(1:xDim,:)      + sum(VVsm(:,:,1:T-1),3) + MUsm0*MUsm1';  % Wk,k+1= sigmak,k+1 + xk*xk+1
    S00(1:xDim,1:xDim) = S00(1:xDim,1:xDim) + sum(Vsm(:,:,1:T-1),3)  + MUsm0*MUsm0';  % Wk
